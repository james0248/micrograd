...
fn engine_cross_entropy_with_logits(logits: &Tensor, targets: &[u8]) -> Tensor {
...
fn cross_entropy_backward_row_sums_are_zero() {
    ...
    let loss = engine_cross_entropy_with_logits(&logits, &[2, 0]);
    loss.backward();
    let g = logits.grad();
    assert_eq!(g.len(), 6);
...
fn no_grad_backward_panics() {
    ...
    let result = std::panic::catch_unwind(|| {
        no_grad(|| {
            ...
            let loss = engine_cross_entropy_with_logits(&logits, &[2]);
            loss.backward();
        });
    });
