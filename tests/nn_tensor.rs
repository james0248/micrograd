...
use micrograd::nn_tensor::MlpTensor;
use micrograd::optim::{Optimizer, Sgd};
use micrograd::tensor::{Tensor, no_grad, reset_state, with_grad};
...
fn mean_loss(model: &MlpTensor, rows: &[([f32; 2], u8)]) -> f32 {
    no_grad(|| {
...
fn mlp_tensor_forward_shape_is_batch_by_output() {
    reset_state();
    let model = MlpTensor::new(&[784, 128, 10], 7);
    with_grad(|| {
...
fn tensor_training_smoke_loss_decreases() {
    reset_state();
    let model = MlpTensor::new(&[2, 8, 2], 42);
