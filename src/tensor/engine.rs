...
impl Engine {
    ...
    pub(super) fn exit_context(&mut self, expected: ContextKind) {
        assert!(
            self.tapes.len() > 1,
            "context stack underflow while exiting tensor context"
        );
        let tape = self.tapes.pop().expect("tensor context stack underflow");
        assert_eq!(
            tape.kind, expected,
            "tensor context stack mismatch while exiting context"
        );
    }
    ...
    pub(super) fn clear_graph(&mut self) {
        assert_eq!(
            self.context_depth(),
            0,
            "tensor::clear_graph() is only allowed when no extra scope is active"
        );
    ...
    fn param(&self, idx: usize) -> &ParamNode {
        self.params
            .get(idx)
            .unwrap_or_else(|| panic!("invalid tensor parameter index {idx}"))
    }
